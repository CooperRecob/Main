...
def newSched():
    ...
    tkinter.Label(text="Monday").grid(row=1, column=0)
    tkinter.Label(text="Tuesday").grid(row=1, column=1)
    tkinter.Label(text="Wednesday").grid(row=1, column=2)
    tkinter.Label(text="Thursday").grid(row=1, column=3)
    ...
    ttk.OptionMenu(window2, var5, "default", *tlist).grid(row=2, column=4)
...
tkinter.Button(text="Submit", command=lambda: newSched()).pack()
